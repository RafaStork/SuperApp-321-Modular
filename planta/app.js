...
async function preloadDefaultLogoForExport(){
  if(defaultLogoForExport!==DEFAULT_LOGO)return defaultLogoForExport;
  if(!defaultLogoForExportPromise){
    defaultLogoForExportPromise=fetch(DEFAULT_LOGO,{cache:"force-cache"}).then(async response=>{
      if(!response.ok)throw new Error("Falha ao carregar a logo compartilhada");
      const svgText=await response.text();
      defaultLogoForExport="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(svgText);
      return defaultLogoForExport;
    }).catch(error=>{
      defaultLogoForExportPromise=null;
      throw error;
    });
  }
  return defaultLogoForExportPromise;
}
...
function closeModal(){
  ...
  setTimeout(()=>{ modalBody.classList.remove("q-wide"); modalBody.classList.remove("pm-modal"); modalBody.classList.remove("pdf-export-modal"); }, 200);
}
...
// A logo padrão fica centralizada em shared. O caminho não é repetido dentro
// dos JSONs salvos; ao carregar, a ausência do campo volta para DEFAULT_LOGO.
// Logos customizadas continuam sendo persistidas normalmente.
...
function makePlantSheetSvgBold(svgText){
...
function buildCarimboSVG(mLeft, rW, mBot, H, carimboH, scaleLabel, plantaLabel, folhaLabel){
  ...
  const logoHref=m.logo===DEFAULT_LOGO?defaultLogoForExport:m.logo;
  if(logoHref) {
    tb+=`<image x="${TX+2}" y="${ty+1}" width="50" height="13" href="${logoHref}" preserveAspectRatio="xMinYMid meet"/>`;
  } else {
...
async function appendAndar2SheetAoPDF(doc, dimMode, incluirLabels){
  const svgStr=makePlantSheetSvgBold(buildSheetSVGInner(dimMode, incluirLabels, 'andar2'));
  doc.addPage();
...
async function appendBlocosSheetAoPDF(doc){
  const svgStr=makePlantSheetSvgBold(buildBlocosSheetSVG());
  doc.addPage();
  const parser=new DOMParser();
  const svgDoc=parser.parseFromString(svgStr,"image/svg+xml");
  const svgEl=svgDoc.documentElement;
