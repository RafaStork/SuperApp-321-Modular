...
function base(){return {exampleVersion:4,simpleInvestment:175000,name:'Chalé Aurora — Estudo Base',client:'Cliente investidor',city:'Destino turístico de serra',model:'A-45 Premium',units:1,area:45,deck:18,guests:4,bedrooms:1,bathrooms:1,start:4,horizon:10,discount:12,inflation:4.5,adrGrowth:5,costGrowth:4.5,appreciation:4,residual:70,capex:{Terreno:[['Terreno',1,135000],['Documentação, projetos e licenças',1,18000]],'Chalé modular':[['Módulo 321 Modular',1,229000],['Frete, guindaste e montagem',1,23000]],Infraestrutura:[['Fundação e terraplenagem',1,36000],['Água, esgoto, elétrica e internet',1,34000],['Deck, paisagismo e acesso',1,32000]],'Mobiliário e abertura':[['Móveis e eletrodomésticos',1,36000],['Enxoval, decoração e utensílios',1,12000],['Fotografia, anúncio e capital de giro',1,8000]]},contingency:7,revenue:{adr:790,occ:45,blocked:1,stay:2.8,clean:210,other:250,cancel:1.5,seasonAdr:[.82,.84,.9,.96,1.02,1.12,1.28,1.2,1.05,.98,.94,1.18],seasonOcc:[.76,.78,.84,.92,1,1.1,1.25,1.18,1.05,.96,.9,1.16]},ops:{profile:'managed',monthlyCost:3200,fixos:[['Energia, água e gás',780],['Internet, seguro e IPTU',420],['Jardinagem e manutenção externa',400],['Marketing e software',300]],noite:[['Energia adicional por noite',18],['Gás por noite',8]],reserva:[['Limpeza e lavanderia',210],['Amenities e consumíveis',50]],percent:[['Gestão da propriedade',10],['Plataforma — taxa única profissional',16],['Impostos sobre faturamento',6],['Reserva de manutenção',4]]},finance:{enabled:true,entry:20,rate:25,term:84,grace:0,system:'PRICE',fees:6500,capitalized:false},scenario:'base',scenarios:{conservative:{occ:-10,adr:-5,capex:0,delay:1,cost:1},base:{occ:0,adr:0,capex:0,delay:0,cost:0},optimistic:{occ:15,adr:8,capex:0,delay:0,cost:-3}}}}
...
let horizonFloat=null,horizonFloatCleanup=null;
function closeHorizonMenu(){if(horizonFloatCleanup)horizonFloatCleanup();if(horizonFloat)horizonFloat.remove();horizonFloat=null;horizonFloatCleanup=null;document.getElementById('horizonToggle')?.setAttribute('aria-expanded','false')}
function positionHorizonMenu(trigger,panel){const rect=trigger.getBoundingClientRect(),height=panel.offsetHeight,width=panel.offsetWidth;let top=rect.bottom+5;if(top+height>window.innerHeight-8)top=Math.max(8,rect.top-height-5);let left=rect.left;if(left+width>window.innerWidth-8)left=Math.max(8,window.innerWidth-width-8);panel.style.top=top+'px';panel.style.left=left+'px'}
function toggleHorizonMenu(event){event.stopPropagation();const trigger=event.currentTarget;if(horizonFloat){closeHorizonMenu();return}const panel=document.createElement('div');panel.className='dd-panel-float horizon-panel';panel.setAttribute('role','listbox');panel.setAttribute('aria-label','Horizonte da análise');[5,10,15,20,25,30,35].forEach(years=>{const option=document.createElement('button');option.type='button';option.className='dd-opt'+(num(data.horizon)===years?' dd-opt-sel':'');option.setAttribute('role','option');option.setAttribute('aria-selected',num(data.horizon)===years?'true':'false');option.textContent=years+' anos';option.addEventListener('click',()=>{data.horizon=years;closeHorizonMenu();save();render()});panel.append(option)});document.body.append(panel);horizonFloat=panel;trigger.setAttribute('aria-expanded','true');positionHorizonMenu(trigger,panel);const onScroll=scrollEvent=>{if(panel.contains(scrollEvent.target))return;closeHorizonMenu()},onResize=()=>positionHorizonMenu(trigger,panel),onDocumentClick=clickEvent=>{if(!panel.contains(clickEvent.target)&&clickEvent.target!==trigger&&!trigger.contains(clickEvent.target))closeHorizonMenu()},onKey=keyEvent=>{if(keyEvent.key==='Escape'){closeHorizonMenu();trigger.focus()}};document.addEventListener('scroll',onScroll,true);window.addEventListener('resize',onResize);setTimeout(()=>document.addEventListener('click',onDocumentClick),0);document.addEventListener('keydown',onKey);horizonFloatCleanup=()=>{document.removeEventListener('scroll',onScroll,true);window.removeEventListener('resize',onResize);document.removeEventListener('click',onDocumentClick);document.removeEventListener('keydown',onKey)}}
function applySimulationTheme(theme,persist=true){const normalized=theme==='dark'?'dark':'light';document.documentElement.dataset.theme=normalized;const toggle=document.getElementById('themeBtn'),knob=toggle?.querySelector('.knob');if(knob)knob.textContent=normalized==='dark'?'🌙':'☀️';if(toggle){toggle.setAttribute('aria-pressed',normalized==='dark'?'true':'false');toggle.setAttribute('aria-label',normalized==='dark'?'Ativar modo claro':'Ativar modo escuro')}if(persist){try{localStorage.setItem('321modular_theme',normalized)}catch(_){}}if(data)renderCharts()}function closeMobileMenu(){document.getElementById('side')?.classList.remove('open');document.getElementById('asideBackdrop')?.classList.remove('active')}
function setupInterface(){
 ...
 document.getElementById('themeBtn').addEventListener('click',()=>applySimulationTheme(document.documentElement.dataset.theme==='dark'?'light':'dark'));
 document.getElementById('horizonToggle').addEventListener('click',toggleHorizonMenu);
 applySimulationTheme(document.documentElement.dataset.theme||'light',false);
...
window.addEventListener('storage',event=>{if(event.key==='321modular_theme')applySimulationTheme(event.newValue||'light',false)});
