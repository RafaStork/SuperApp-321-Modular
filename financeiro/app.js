...
document.getElementById('loginLogo').src = LOGO_321;
...
const SESSION_KEY = '321fin_session';
let SESSION = null;

function showLoginError(msg){ const el=document.getElementById('lgError'); if(el){ el.textContent=msg; el.style.display='block'; } }
...
async function doLogin(){ location.href=window.SuperAppAuth.getPortalUrl(); }
async function doLogout(){ location.href = window.SuperAppAuth.getPortalUrl(); }
async function tryAutoLogin(){ await loadCentralFinanceSession(); }
let DB = {
  obras:[], receber:[], pagar:[],
...
// load() não é mais usado para iniciar o app (ver doLogin/tryAutoLogin) — mantido só por compatibilidade
function load(){}
